...
var phoneticimg=['https://i.imgur.com/ZG1N0BK.png','https://i.imgur.com/YAR5VyV.png','https://i.imgur.com/0VduCZw.png','https://i.imgur.com/U8t0MPP.png','https://i.imgur.com/YQZZqF7.png','https://i.imgur.com/OAG8pmL.png','https://i.imgur.com/tA9prIy.png','https://i.imgur.com/d6ByDJ6.png']
var langnum=0 //0華文 1KIP漢羅 2白話字
var fontimg=[['https://i.imgur.com/tC8zZEB.png','https://i.imgur.com/V6ubfSJ.png','https://i.imgur.com/1ybmM9G.png','https://i.imgur.com/gben6cA.png'],['https://i.imgur.com/AeSTnzm.png','https://i.imgur.com/CAMogPD.png','https://i.imgur.com/J2b95Iq.png'],['https://i.imgur.com/1zaV7jZ.png','https://i.imgur.com/mfPtOtA.png','https://i.imgur.com/vOlpd6J.png'],['https://i.imgur.com/I3CeWLK.png'],['https://i.imgur.com/8COXxoY.png']]

//phonetic為三維陣列 代入fontnum、weightnum、phoneticnum之後的數字對應到標音
phonetic[0]=[[0,-1,2,-1,4,-1,-1,-1],[0,1,2,3,4,5,6,7],[0,1,2,3,4,-1,-1,-1],[0,-1,2,-1,4,-1,-1,-1]];//源石黑體LRBH的標音表
phonetic[1]=[[0,-1,2,-1,4,-1,-1,-1],[0,1,2,3,4,-1,-1,-1],[0,1,2,3,4,-1,-1,-1]]
phonetic[2]=[[0,1,2,3,4,5,6,7],[0,1,2,3,4,-1,-1,-1],[0,-1,2,-1,4,-1,-1,-1]]
phonetic[3]=[[0,1,2,3,4,-1,-1,-1]]
phonetic[4]=[[0,1,2,3,4,5,6,7]]
var phoneticnum=4

//重製漢字圖片
function fontimgg(){
  $(".hj").attr('src',fontimg[fontnum][weightnum]);
  return 0;
}
...
$(".zh").click(
  function(){
    if(langnum!=0){
      ...
      $(".t1").html("提供 5 種字體風格、 8 種標音組合與不同粗細<br>共 61 套字型的台文標音字體大家族");
      $(".t2_1").text("下滑看更多介紹，右上角可以切換語言 ٩( 'ω' )و");
      $(".t2_2").html("下滑看更多介紹<br>右上角可以切換語言 ٩( 'ω' )و");
      $(".t3_1").text("點擊右方選單，看看有哪些字型組合 (๑•̀ω•́)ノ");
      $(".t3_2").text("目前網頁介紹尚無支援手機板<br>請用電腦瀏覽完整內容 ┗( T﹏T )┛");
...
$(".kip").click(
  function(){
    if(langnum!=1){
      ...
      $(".t1").html("提供 5 種風格、8 種標音組合佮無仝字重<br>攏總 61 套字型 ê 台文標音字體大家族");
...
$(".fontprev").click(
  function(){
    //存取舊字重、標音
    var oldfw = weightdata[fontweight[fontnum][weightnum]]
    var oldph = phoneticdata[phonetic[fontnum][weightnum][phoneticnum]]
    fontnum-=1;
    if(fontnum==-1){
      fontnum=4;
    };
    ...
    fw(fontnum);
    //重置字重判斷
    if(weightdata[fontweight[fontnum][weightnum]]!=oldfw){
      weightnum=0;
      $(".fontweight").text(weightdata[fontweight[fontnum][weightnum]]);
    }
    //重置標音判斷
...
$(".fontnext").click(
  function(){
    ...
    fw(fontnum);
    //重置字重判斷
    var oldfw = weightdata[fontweight[oldfn][weightnum]]
    if(weightdata[fontweight[fontnum][weightnum]]!=oldfw){
      weightnum=0;
      $(".fontweight").text(weightdata[fontweight[fontnum][weightnum]]);
    }
...
$(".weightprev").click(
  function(){
    ...
    weightnum-=1
    if(weightnum==-1){
      weightnum=fontweight[fontnum].length-1;
      //fontweight[fontnum].length-1是指該字體字重的數量
    }
   $(".fontweight").text(weightdata[fontweight[fontnum][weightnum]]);
    //重置標音判斷
    if(phonetic[fontnum][weightnum][phoneticnum]==-1){
...
$(".weightnext").click(
  function(){
    ...
    var oldph = phoneticdata[phonetic[fontnum][weightnum][phoneticnum]]
    weightnum+=1
    if(weightnum==fontweight[fontnum].length){
      weightnum=0;
    }
    $(".fontweight").text(weightdata[fontweight[fontnum][weightnum]]);
    //重置標音判斷
